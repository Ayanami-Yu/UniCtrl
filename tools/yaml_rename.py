...
modality = "videos"  # images or videos
config_file = (
    "metrics/images_v2.yaml" if modality == "images" else "metrics/videos_v2.yaml"
)
...
for mode in data.keys():
    for name in data[mode].keys():
        for method in data[mode][name][sub_dir].keys():
            ...
            path_new = (
                f"metrics/{modality}/tgt/{method}/{mode}/{name}_{method}" + postfix
            )
