...
def register_attention_editor_diffusers(model, editor: AttentionBase):
    """
    Register an attention editor to Diffuser Pipeline, refer from [Prompt-to-Prompt]
    """

    # NOTE encoder_hidden_states of shape (2B, 77, 1024) is text_embeddings
    def ca_forward(self, place_in_unet):
        def forward(
            ...
            attn = sim.softmax(dim=-1)  # (BH, 4096, 77)
            # the only difference
            out = editor(
                q,  # (2Bh, HW, d // h)
                k,
...
def regiter_attention_editor_ldm(model, editor: AttentionBase):
    """
    Register an attention editor to Stable Diffusion model, refer from [Prompt-to-Prompt]
    """
